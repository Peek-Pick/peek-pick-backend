...
public interface ProductTagRepository extends JpaRepository<ProductTagEntity, Long> {
    Optional<ProductTagEntity> findByProductBaseEntityAndTagEntity(ProductBaseEntity productBaseEntity, TagEntity tagEntity);

    // 특정 상품의 가장 많이 사용된 태그 1개 가져오기
    @Query("SELECT pt.tagEntity.tagName FROM ProductTagEntity pt WHERE pt.productBaseEntity.productId = :productId ORDER BY pt.tagCount DESC LIMIT 1")
